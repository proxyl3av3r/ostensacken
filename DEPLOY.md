...
Меняли только статику (html/css/js/картинки)? → достаточно `git pull`, `pm2 restart` не нужен.

> ⚠️ **НЕ копируй `deploy/nginx.conf` поверх рабочего конфига на сервере с уже настроенным HTTPS.**
> `deploy/nginx.conf` — это HTTP-шаблон (порт 80). Certbot при выпуске сертификата дописывает в живой
> конфиг блок `:443` и редирект. `cp deploy/nginx.conf /etc/nginx/sites-available/...` затирает этот
> блок → сайт остаётся только на `:80`, а браузер (из-за прошлых HTTPS-заходов) стучится на `:443` и
> получает `ERR_CONNECTION_REFUSED`. `nginx -t` при этом проходит — ошибки не видно.
> Если всё же перезаписал — восстанови HTTPS:
> ```bash
> certbot --nginx -d ostensacken.com -d www.ostensacken.com --redirect --non-interactive --agree-tos -m rishelie2003@gmail.com
> ```
> Нужно поменять сами настройки nginx (заголовки и т.п.)? Правь `/etc/nginx/sites-available/ostensacken.com`
> вручную внутри существующих блоков, потом `nginx -t && systemctl reload nginx`.

### Кэш статики
К подключениям css/js добавлен `?v=N` (см. `index.html`, `admin/index.html`). Если правишь css/js и хочешь,
чтобы у всех сразу подхватилось (в nginx на ассеты стоит `expires 30d`) — **подними номер версии** `?v=`
на изменённых подключениях. Иначе браузеры будут держать старый файл до 30 дней.

---

## Адмін-панель (/admin)

Вхід за паролем. Розділи: **Замовлення** (перегляд + статуси), **Ціни**, **FAQ**, **Тексти** (Hero/Про нас/контакти), **Фото** (заміна зображень сайту).
Замовлення — у `data/orders.json`, контент — у `data/content.json`, завантажені фото — у `public/images/uploads/`.

1. Згенеруй **хеш пароля** (пароль у відкритому вигляді ніде не зберігається):
   ```bash
   cd /var/www/ostensacken
   node scripts/hash-password.js "ваш-складний-пароль"
   ```
   Скопіюй рядок `ADMIN_PASSWORD_HASH=...` у `.env`. Поле `ADMIN_PASSWORD` лиши порожнім.
2. Перезапусти: `pm2 restart ostensacken --update-env`
3. Відкрий **https://ostensacken.com/admin**, увійди паролем.

> **Безпека:** пароль зберігається як scrypt-хеш; сесії серверні (cookie містить лише випадковий id, тому «Вийти» реально інвалідовує сесію); усі зміни в адмінці захищені **CSRF**-токеном; cookie HttpOnly + SameSite=Lax (+Secure на HTTPS). Ліміти: вхід 10/10 хв, зміни 120/10 хв. Завантаження фото — лише PNG/JPG/WEBP до 6 МБ, з перевіркою сигнатури файлу.

> **Важливо при оновленні:** `data/` та `public/images/uploads/` — у `.gitignore`, тож `git pull` їх не чіпає (замовлення, контент і завантажені фото зберігаються між деплоями). Каталоги створюються автоматично при першому записі.
