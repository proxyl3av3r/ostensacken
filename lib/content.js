...
const DEFAULTS = {
  // Усі керовані тексти сайта. Ключі з накладанням дзеркалять data-text у index.html.
  // У «rich»-заголовках слово в *зірочках* рендериться акцентом (червоним) — див. site.js.
  texts: {
    site_title: 'Титанові глушники для нарізної зброї',
    // Hero
    hero_title: 'Титанові глушники *під замовлення*',
    hero_subtitle: 'Титанові глушники на всі види нарізної зброї. Максимальна точність, надійність і бездоганна якість виконання.',
    // Показники
    stat1_value: 'Для всіх', stat1_label: 'Популярних калібрів',
    stat2_label: 'Виконаних замовлень',
    stat3_value: '10+ років', stat3_label: 'На ринку України',
    // Типи глушників
    types_title: 'Виберіть *тип* глушника',
    prod_standard_title: 'Стандартний',
    prod_standard_info: 'Характеристики актуальні для глушника у калібрі .223 та .308',
    prod_integrated_title: 'Інтегрований',
    prod_integrated_info: 'Глушник у калібрі .338 робиться у діаметрі 54мм та приблизна вага сягає 1000г',
    // Калібри
    compat_title: 'Сумісні з усіма популярними *калібрами*',
    // Вартість
    pricing_title: 'Актуальна *вартість*',
    // 3 кроки
    steps_title: '3 простих *кроки*',
    steps_sub: 'Як зробити замовлення?',
    step1_title: 'Оберіть потрібний глушник',
    step1_desc: 'Виберіть тип глушника (стандартний або інтегрований), потрібний калібр та хід різьби.',
    step2_title: 'Заповніть контактні дані',
    step2_desc: 'Вкажіть ваше ім\'я, номер телефону та адресу електронної пошти для зворотного зв\'язку.',
    step3_title: 'Підтвердьте замовлення',
    step3_desc: 'Перевірте правильність введених даних та натисніть кнопку «Замовити». Після отримання заявки ми зв\'яжемося з вами для підтвердження та уточнення всіх деталей.',
    // Про нас
    about_title: 'Про *нас*',
    about_brand: 'osten - sacken',
    ...
    // Контакти / питання
    order_title: 'Залишились *питання?*',
    order_desc: 'Зв\'яжіться з нами — ми допоможемо підібрати оптимальне рішення, відповімо на всі технічні питання та проконсультуємо щодо замовлення.',
  ...
  stock: {},
  // Відео-відгуки: список файлів + скільки показувати на сайті (limit=0 → усі).
  reviews: {
    videos: ['videos/review-1.mp4', 'videos/review-2.mp4', 'videos/review-3.mp4', 'videos/review-4.mp4'],
    limit: 0
  }
};
...
function getContent() {
  ...
  if (saved.reviews && typeof saved.reviews === 'object') {
    if (Array.isArray(saved.reviews.videos)) {
      out.reviews.videos = saved.reviews.videos.filter((x) => typeof x === 'string' && x).slice(0, MAX_REVIEWS);
    }
    const lim = parseInt(saved.reviews.limit, 10);
    if (Number.isFinite(lim) && lim >= 0) out.reviews.limit = Math.min(MAX_REVIEWS, lim);
  }
  return out;
}
...
/* ---- Відео-відгуки (додавання/видалення файлів + скільки показувати) ---- */
function addReview(relPath) {
  const saved = readRaw();
  const cur = getContent().reviews.videos;   // поточний список (з дефолтами)
  if (cur.length >= MAX_REVIEWS) throw new Error('Досягнуто ліміт відео (' + MAX_REVIEWS + ')');
  const next = cur.concat([String(relPath)]);
  saved.reviews = Object.assign({ limit: getContent().reviews.limit }, saved.reviews);
  saved.reviews.videos = next;
  writeRaw(saved);
  return getContent().reviews;
}
// повертає { reviews, removed } — removed використовується сервером для видалення файлу
function removeReview(index) {
  const saved = readRaw();
  const cur = getContent().reviews.videos.slice();
  const i = parseInt(index, 10);
  if (!(i >= 0 && i < cur.length)) throw new Error('Невірний індекс відео');
  const removed = cur.splice(i, 1)[0];
  saved.reviews = Object.assign({ limit: getContent().reviews.limit }, saved.reviews);
  saved.reviews.videos = cur;
  writeRaw(saved);
  return { reviews: getContent().reviews, removed: removed };
}
function setReviewsLimit(n) {
  const lim = Math.max(0, Math.min(MAX_REVIEWS, parseInt(n, 10) || 0));
  const saved = readRaw();
  saved.reviews = Object.assign({ videos: getContent().reviews.videos }, saved.reviews);
  saved.reviews.limit = lim;
  writeRaw(saved);
  return getContent().reviews;
}

module.exports = {
  DEFAULTS, IMAGE_SLOTS, TEXT_KEYS, PRODUCTS, GALLERY_KEYS, MAX_GALLERY, MAX_REVIEWS, STOCK_SEP,
  ...
  addReview, removeReview, setReviewsLimit
};
